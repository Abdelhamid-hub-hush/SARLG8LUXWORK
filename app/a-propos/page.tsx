...
export default function AboutPage() {
  ...
  const t = {
    ...
    stats: [
      { k: lang === "fr" ? "Projets réalisés" : "مشاريع منجزة", v: "120+" },
      { k: lang === "fr" ? "Clients satisfaits" : "زبناء راضين", v: "100+" },
      { k: lang === "fr" ? "Années d’expérience" : "سنوات الخبرة", v: "7+" },
    ],
    ...
    steps: [
      {
        ...
        v: lang === "fr" ? "Analyse du projet et conseils techniques." : "فهم المشروع وتقديم النصائح.",
      },
      {
        k: lang === "fr" ? "2. Devis" : "2. عرض الثمن",
        v: lang === "fr" ? "Proposition claire et détaillée." : "عرض ثمن واضح ومفصل.",
      },
      {
        k: lang === "fr" ? "3. Travaux" : "3. الأشغال",
        v: lang === "fr" ? "Exécution organisée avec contrôle qualité." : "تنفيذ الأشغال بجودة وتنظيم.",
      },
      {
        k: lang === "fr" ? "4. Livraison" : "4. التسليم",
        v: lang === "fr" ? "Nettoyage et validation finale." : "تنظيف وتسليم المشروع.",
      },
    ],
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-12">
      <div className="rounded-[32px] border border-white/10 bg-white/[0.02] p-10 shadow-soft">
        ...
        <h1 className="mt-2 text-3xl md:text-4xl font-semibold">{t.title}</h1>
        <p className="mt-4 max-w-3xl text-white/70 leading-relaxed">{t.intro}</p>
      </div>

      <div className="mt-10 grid grid-cols-3 gap-4 text-center">
        {t.stats.map((s) => (
          <div key={s.k} className="rounded-2xl border border-white/10 bg-white/[0.02] p-6">
      ...
      <div className="mt-10">
        ...
        <div className="mt-6 grid md:grid-cols-3 gap-4">
          {t.services.map((s) => (
            <div key={s} className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
